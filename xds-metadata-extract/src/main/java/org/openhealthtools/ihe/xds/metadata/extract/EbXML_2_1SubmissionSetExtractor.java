 ...
 *
 */
public class EbXML_2_1SubmissionSetExtractor implements SubmissionSetExtractor {
	
	/** Descriptor for Extractor */
	public static final String DESCRIPTOR = "EBXML_SS_EXTR";
	
	/** Logger */
	private static final Logger logger = Logger.getLogger(EbXML_2_1SubmissionSetExtractor.class);
	...
	private void extractRegistryPackageAttributes(SubmissionSetType subSet) {
		...
		if (this.setData.getStatus() != null) {
			if (this.setData.getStatus().equals(StatusType.APPROVED_LITERAL)) {
				subSet
						.setAvailabilityStatus(AvailabilityStatusType.APPROVED_LITERAL);
			} else if (this.setData.getStatus()
					.equals(StatusType.DEPRECATED_LITERAL)) {
				subSet
						.setAvailabilityStatus(AvailabilityStatusType.DEPRECATED_LITERAL);
			} else if (this.setData.getStatus().equals(StatusType.SUBMITTED_LITERAL)) {
				subSet
						.setAvailabilityStatus(AvailabilityStatusType.SUBMITTED_LITERAL);
			} else if (this.setData.getStatus().equals(StatusType.WITHDRAWN_LITERAL)) {
				subSet
