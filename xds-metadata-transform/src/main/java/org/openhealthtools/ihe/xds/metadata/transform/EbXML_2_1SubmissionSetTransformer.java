 ...
 * metadata to ebRIM 2.1 elements.
		...
		if(subSet.getAssociatedDocuments() != null){
			if(!subSet.getAssociatedDocuments().isEmpty()){
				...
				while(i.hasNext()){
					...
					this.documentsInSubmissionSet[count].setAssociationType(UUIDs.HAS_MEMBER_UUID);
		...
		if(subSet.getAssociatedFolders() != null){
			if(!subSet.getAssociatedFolders().isEmpty()){
				...
				while(i.hasNext()){
					...
					this.foldersInSubmissionSet[count].setAssociationType(UUIDs.HAS_MEMBER_UUID);
