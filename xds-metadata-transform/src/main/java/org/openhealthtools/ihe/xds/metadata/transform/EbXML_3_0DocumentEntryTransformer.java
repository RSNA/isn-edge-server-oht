...
/**
 * Class containing utility methods to transform XDS document entry metadata to
 * ebRIM 3.0 element.
 *
...
public class EbXML_3_0DocumentEntryTransformer implements DocumentEntryTransformer
{
	...
	/**
	 * Returns the ExtrinsicObjectType containing the document entry metadata
	 * values. Returns null if the transformation has not been executed, or had
	 * failed.
	 *
	...
	/**
	 * Returns the AssociationType1 containing the document entry metadata
	 * values pertinant to the parent document, if present Returns null if the
	 ...
	 * containing the document entry metadata values of the parameter, formatted
	 * in ebXML
	 *
	 * @throws MetadataTransformationException
	 * @see
	 * org.openhealthtools.ihe.xds.metadata.transform.DocumentEntryTransformer#transform(org.openhealthtools.ihe.xds.metadata.DocumentEntryType)
	 */
	 ...
	 * cause issues if this entryUUID did not come come from the Registry. Also
	 * sets ExtrinsicObject/@objectType to
	...
	private void setExtrinsicObjectSlots(DocumentEntryType docEntry)
	{
		...
		EList<CXi> refIdList = docEntry.getReferenceIdList();
		if (refIdList != null && !refIdList.isEmpty()) {
			...
			list = RimFactory.eINSTANCE.createValueListType();
			slot.setValueList(list);
			slot.setName(DocumentEntryConstants.REFERENCE_ID_LIST);


			for (CXi id : refIdList) {
				String hl7 = HL7V2MessageFormat.toMessageString(
						id,
						MessageDelimiters.COMPONENT,
						MessageDelimiters.SUBCOMPONENT);

				slot.getValueList().getValue().add(hl7);
			}
	...
	/**
	 * private utility to aid in copying InternationalStrings in metadata to
	 * those in ebXML.
	 */
